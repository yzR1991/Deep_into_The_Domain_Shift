...
def CD_KL(x,y):
    ...
    for i in range(x.shape[1]-1):
        for j in range(i,x.shape[1]):
            ...
            cx_ij = Kendall_tau(x[:,i], x[:,j], size)
            cy_ij = Kendall_tau(y[:,i], y[:,j], size)
            cd += (torch.log(1 - cx_ij**2) - torch.log(1 - cy_ij**2))**2
            # cd += torch.abs(torch.log(1 - cx_ij**2) - torch.log(1 - cy_ij**2))
    return cd

class ClassifierHead(nn.Module):
    """
    classifiers of target for MCD
    """
...
def classifier_discrepancy(predictions1: torch.Tensor, predictions2: torch.Tensor):
